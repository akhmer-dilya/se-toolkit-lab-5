...
async def get_scores(
    ...
    lab_stmt = select(ItemRecord).where(
        ItemRecord.type == "lab", ItemRecord.title.ilike(lab_title_pattern)
    )
    ...
    task_ids = [r for r in tasks_result.all()]
    ...
    stmt = (
        ...
        .where(InteractionLog.item_id.in_(task_ids), InteractionLog.score.isnot(None))
...
async def get_pass_rates(
    ...
    tasks_stmt = (
        select(ItemRecord)
        .where(ItemRecord.type == "task", ItemRecord.parent_id == lab_item.id)
    ...
    result_list = []
    for task in tasks:
        # Get avg score and count for this task
        stats_stmt = select(
